...
int main()
{
	...
	try
	{
		...
		printf("        %s\n", toString(93326215443944152681699238856266700490715968264381621468592963895217599993229915608941463976156518286253697920827223758251185210916864000000000000000000000000_bn1024).c_str());
		printf("        %s\n", toString(fromString<1024>("93326215443944152681699238856266700490715968264381621468592963895217599993229915608941463976156518286253697920827223758251185210916864000000000000000000000000")).c_str());
		...
		printf("BigNum::Num<1024>(1) << 1023 =  %s\n", toString(BigNum::Num<1024>(1) << 1023).c_str());
...

		{
			...
			for (int i = 0; i < 64; i++)
			{
				...
				passed = passed && (testbn[i / BigNum::DigitSizeBits] == (1u << i%BigNum::DigitSizeBits));
			}
			test(passed == true);
		}
...

		{
			...
			{
				BigNum::Num<64> testbn = bn >> i;
				passed = passed && (testbn[(63 - i) / BigNum::DigitSizeBits] == (1u << ((BigNum::DigitSizeBits - 1) - i%BigNum::DigitSizeBits)));
			}
